...
ENTRYPOINT ["sh", "-c", "java -jar -Dspring.datasource.url=jdbc:mysql://${MYSQL_URL}:${MYSQL_PORT}/${MYSQL_DATABASE} -Dspring.datasource.username=${MYSQL_USER} -Dspring.datasource.password=${MYSQL_PASSWORD} -Dinventory.inventory-file=${INVENTORY_FILE} -Dinventory.products-file=${PRODUCTS_FILE} app.jar"]
